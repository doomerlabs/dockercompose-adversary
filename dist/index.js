#!/usr/bin/env node
import { createRequire as __dockerComposeCreateRequire } from 'node:module'; const require = __dockerComposeCreateRequire(import.meta.url);
...
var MAX_OBS = 6;
var DOCKERCOMPOSE_MODEL_PROMPT = `You are reviewing Docker Compose files for privilege, host access, image reproducibility, and secret exposure.

Authority: privileged services, host network/pid/ipc, mutable tags, secret mounts, dangerous volume binds.
...
var DOCKERCOMPOSE_MODEL_SCHEMA = {
  type: "object",
  additionalProperties: false,
    ...
    observations: {
      ...
      items: {
        ...
        properties: {
          ...
          category: { type: "string", enum: ["privileged", "host-namespace", "mutable-image", "secrets", "network", "completeness"] },
...
` : source.content;
  ...
  return {
    ...
    request: {
      prompt: DOCKERCOMPOSE_MODEL_PROMPT,
      input,
      schema: DOCKERCOMPOSE_MODEL_SCHEMA,
      budget: { maximumOutputTokens: 4096, timeoutMs: 12e4 }
    }
  };
}
async function runModelDockerComposeReview(ctx, detections, sources, staticSeverities = [], staticPrimaryConcern) {
  ...
  try {
    ...
    await applyModelDockerComposeReview(ctx, result.output, evidenceById, staticSeverities, staticPrimaryConcern);
    return "applied";
  } catch (error) {
...
async function applyModelDockerComposeReview(ctx, output, evidenceById, staticSeverities, staticPrimaryConcern) {
  ...
  for (const observation of output.observations.slice(0, MAX_OBS)) {
    ...
    ctx.review.observe({
      key: `dockercompose.model.${observation.id}`,
...
async function analyzeRepository(ctx) {
  ...
  await runModelDockerComposeReview(
    ctx,
    detections.map((d) => ({
      ruleId: d.rule.id,
      file: d.file,
...
function evaluate(rule, sources, allPaths) {
  ...
  if (match.kind === "missing-content") {
    return matchingSources.flatMap((file) => {
      ...
      const location = locate(file.source, match.trigger, (line) => isEligibleLine(file, line));
  ...
  return matchingSources.flatMap((file) => {
    ...
    const location = locate(file.source, match.pattern, (line) => isEligibleLine(file, line));
    if (location === void 0) return [];
...
function locate(source, expression, eligible) {
